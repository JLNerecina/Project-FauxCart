...
export function OrderHistorySection({ purchases }: { purchases: PastPurchase[] }) {
  ...
  return (
    <section className="mt-24 border-t border-slate-200 pt-16 pb-24">
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">Review your simulated past purchases.</p>
          </div>
        </div>

        <div className="space-y-6">
          {purchases.map((purchase) => (
            <div key={purchase.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="bg-slate-50 px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200">
                <div className="flex flex-wrap items-center gap-6 text-sm">
                  <div>
                    <span className="block text-slate-500 font-medium mb-1">Order Placed</span>
                  ...
                  <div>
                    <span className="block text-slate-500 font-medium mb-1">Total</span>
                    <span className="text-slate-900 font-bold">${purchase.total.toFixed(2)}</span>
                  </div>
                  <div>
                    <span className="block text-slate-500 font-medium mb-1">Order #</span>
                    <span className="text-slate-900 font-medium">{purchase.id.toUpperCase()}</span>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  {purchase.items.map((item) => (
                    <div key={item.product.id} className="flex gap-4 sm:gap-6">
                      <div className="relative h-20 w-20 sm:h-24 sm:w-24 shrink-0 bg-slate-50 rounded-xl border border-slate-100 p-2 flex items-center justify-center">
                        <Image
                      ...
                      <div className="flex-1 min-w-0 flex flex-col sm:flex-row justify-between gap-4">
                        <div className="space-y-1">
                          <h4 className="text-base font-semibold text-slate-900">{item.product.title}</h4>
                          <p className="text-sm text-slate-500 line-clamp-1">{item.product.description}</p>
                          <div className="pt-2 text-sm font-medium text-slate-600">
                            Qty: {item.quantity}
                          </div>
                        </div>
