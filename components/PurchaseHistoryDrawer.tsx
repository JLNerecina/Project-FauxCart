        ...
        <>
          ...
          >
            <div className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800">
              ...
              <div className="flex overflow-x-auto hide-scrollbar border-t border-slate-100 dark:border-slate-800">
                {tabs.map(tab => (
                  <button
                    key={tab.id}
                    ...
                    className={`flex-1 min-w-[80px] py-3 text-sm font-medium text-center border-b-2 transition-colors ${
                      activeTab === tab.id 
                        ...
                        : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:bg-slate-950'
                    }`}
                  >
                    {tab.label}
                  </button>
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
              {filteredPurchases.length === 0 ? (
                <div className="flex h-full flex-col items-center justify-center space-y-4 text-center text-slate-500 dark:text-slate-400">
                  ...
                  <p className="text-lg font-medium text-slate-900 dark:text-slate-100">No {activeTab !== 'all' ? tabs.find(t => t.id === activeTab)?.label : 'purchase'} history</p>
                ...
                filteredPurchases.map((purchase) => (
                  <motion.div
                    ...
                    className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden cursor-pointer hover:shadow-md transition-shadow"
                    onClick={() => onPurchaseClick(purchase)}
                  >
                    <div className="bg-slate-100 dark:bg-slate-800 px-4 py-3 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
                      ...
                      {purchase.items.map((item) => (
                        <div key={item.product.id} className="flex gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <h4 className="text-sm font-medium text-slate-900 dark:text-slate-100 line-clamp-1">{item.product.title}</h4>
                            <div className="flex items-center justify-between mt-1">
                              <span className="text-xs text-slate-500 dark:text-slate-400">Qty: {item.quantity}</span>
                              <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">${(item.product.price * item.quantity).toFixed(2)}</span>
                            </div>
                          </div>
                        </div>
                      ))}
                    ...
                    {purchase.status === 'to_pay' && (
                      <div className="px-4 py-3 bg-slate-50 dark:bg-slate-950 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                        <span className="text-xs text-slate-500 dark:text-slate-400">Wallet: ${walletBalance.toFixed(2)}</span>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
            ...
            {purchases.length > 0 && (
              <div className="bg-white dark:bg-slate-900 p-6 border-t border-slate-200 dark:border-slate-800">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-slate-500 dark:text-slate-400 font-medium">Total Simulated Savings</span>
